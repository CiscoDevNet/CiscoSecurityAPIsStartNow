...
print(f"\n==> Removes an endpoint to a quarantine ANC group")
...
if __name__ == "__main__":
    ...
    url = f"https://{hostname}:9060/ers/config/ancendpoint/clear"
    ...
    try:
        ...
        if (response.status_code  == 204):
            print(f"MAC Address {mac_address} successfully removed from {anc_policy} ANC policy.")
        else:
            print(f"Error {response.status_code_} returned from ISE.") 
    except:
